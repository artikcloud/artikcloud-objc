...
#import "ACBasicAuthTokenProvider.h"
#import "ACLogger.h"

@interface ACDefaultConfiguration ()
...
@implementation ACDefaultConfiguration

#pragma mark - Singleton Methods

+ (instancetype) sharedConfig {
    static ACDefaultConfiguration *shardConfig = nil;
...
- (instancetype) init {
    self = [super init];
    if (self) {
        _host = @"https://api.artik.cloud/v1.1";
        _username = @"";
        _password = @"";
        _accessToken= @"";
        _verifySSL = YES;
...
- (NSString *) getBasicAuthToken {
    ...
    NSString *basicAuthToken = [ACBasicAuthTokenProvider createBasicAuthTokenWithUsername:self.username password:self.password];
