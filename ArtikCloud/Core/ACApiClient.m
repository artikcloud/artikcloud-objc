...
#import "ACDefaultConfiguration.h"

NSString *const ACResponseObjectErrorKey = @"ACResponseObject";

static NSString * const kACContentDispositionKey = @"Content-Disposition";
...
static NSString * AC__fileNameForResponse(NSURLResponse *response) {
    ...
    if(!headers[kACContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kACContentDispositionKey];
...
@property (nonatomic, strong, readwrite) id<ACConfiguration> configuration;
...
+ (instancetype) sharedClient {
    static ACApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[ACDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[ACDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<ACConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[ACResponseDeserializer alloc] init];
        _sanitizer = [[ACSanitizer alloc] init];
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<ACConfiguration> config = self.configuration;
